...
int	for_b(t_node *a, t_node *b, t_utils utils)
{
	...
	{
		ft_putstr_fd(utils.p[SB](&a, &b, &utils), 1);
		return (1);
	}
	else if (b->next->data < d)
	{
		ft_putstr_fd(utils.p[RRB](&a, &b, &utils), 1);
		return (1);
	}
	else if (b->data < d)
	{
		ft_putstr_fd(utils.p[RB](&a, &b, &utils), 1);
		return (1);
	}
	return (0);
}
...
t_node	*sort_few_nums(t_node *a, t_node *b, t_utils utils)
{
	...
	while (!check(a, b))
	{
		...
		if (a->next != a && a->data > a->next->data)
			ft_putstr_fd(utils.p[SA](&a, &b, &utils), 1);
		else if (a->next != a && a->next->data > d)
			ft_putstr_fd(utils.p[RRA](&a, &b, &utils), 1);
		else if (a->next != a && a->data > d)
			ft_putstr_fd(utils.p[RA](&a, &b, &utils), 1);
		else if (check_a(a) && !b->is_empty)
			ft_putstr_fd(utils.p[PA](&a, &b, &utils), 1);
		else if (!a->is_empty)
			ft_putstr_fd(utils.p[PB](&a, &b, &utils), 1);
...
void	get_elm_to_top(t_node **a, t_node **b, t_utils *utils, int elm)
{
	...
	{
		if (i == 1)
			ft_putstr_fd(utils->p[SA + utils->is_b](a, b, utils), 1);
		else if (i <= mid)
			ft_putstr_fd(utils->p[RA + utils->is_b](a, b, utils), 1);
		else if (i > mid)
			ft_putstr_fd(utils->p[RRA + utils->is_b](a, b, utils), 1);
		i = assing_i(*a, *b, *utils, elm);
	}
	ft_putstr_fd(utils->p[PB + utils->is_b](a, b, utils), 1);
}
